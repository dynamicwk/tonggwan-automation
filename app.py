...
# ==========================================
# 📑 TAB 1: 세관 통관 부가세 검증 마스터 (원래 양식 완벽 복원)
# ==========================================
with tab1:
            ...
            if not customs_excel or not decl_pdfs:
                ...
                with st.spinner("🤖 수입신고필증 PDF 데이터 정밀 추출 및 부가세/관세 대조 검증 연산 중..."):
                    try:
                        ...
                        # 열 이름 동적 탐색 (신고번호, 관세, 부가세, 과세가격)
                        col_decl = next((c for c in df_customs.columns if "신고" in str(c) or "번호" in str(c)), df_customs.columns[0])
                        col_vat = next((c for c in df_customs.columns if "부가" in str(c) or "VAT" in str(c).upper()), None)
                        col_duty = next((c for c in df_customs.columns if "관세" in str(c) and "가격" not in str(c)), None)
                        col_val = next((c for c in df_customs.columns if "과세" in str(c) or "가격" in str(c)), None)
                        ...
                        # 4) 검증 및 차액 연산
                        # 고지목록 상의 금액 추출 (없을 경우 0 처리)
                        excel_val = df_merged[col_val].fillna(0).astype(int) if col_val else 0
                        excel_duty = df_merged[col_duty].fillna(0).astype(int) if col_duty else 0
                        excel_vat = df_merged[col_vat].fillna(0).astype(int) if col_vat else 0
                        
                        df_merged["과세가격_차액"] = excel_val - df_merged["필증_과세가격"].fillna(0).astype(int)
                        df_merged["관세_차액"] = excel_duty - df_merged["필증_관세"].fillna(0).astype(int)
                        df_merged["부가세_차액"] = excel_vat - df_merged["필증_부가세"].fillna(0).astype(int)
                        
                        # [검증결과] 판별 로직
                        def verify_status(row):
                            if pd.isna(row["필증파일명"]) or str(row["필증파일명"]).strip() == "":
                                return "⚠️ 필증누락"
                            elif row["부가세_차액"] == 0 and row["관세_차액"] == 0 and row["과세가격_차액"] == 0:
                                return "✅ 일치"
                            else:
                                return "❌ 불일치"
                        ...
                        if col_val: display_cols.extend([col_val, "필증_과세가격", "과세가격_차액"])
                        if col_duty: display_cols.extend([col_duty, "필증_관세", "관세_차액"])
                        if col_vat: display_cols.extend([col_vat, "필증_부가세", "부가세_차액"])
                            ...
                            for r_idx, row in df_result.iterrows():
                                for c_idx, col_name in enumerate(df_result.columns):
                                    ...
                                    if col_name == "검증결과":
                                        ...
                                        ws.write(r_idx + 1, c_idx, val if pd.notna(val) else 0, fmt_num)
                                    else:
                                        ws.write(r_idx + 1, c_idx, val if pd.notna(val) else "", fmt_cell)
